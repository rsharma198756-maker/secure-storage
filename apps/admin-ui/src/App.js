...
export default function App() {
    ...
    const navigateToTab = useCallback((next) => {
        setTab(next);
    ...
    const onCreateUserSubmit = async () => {
        ...
        setIsBusy(true);
        try {
            await createUser(accessToken, newUserEmail, newUserPassword, newUserRole, newUserFirstName, newUserLastName, newUserPhoneNumber);
    ...
    useEffect(() => {
        const onPop = (e) => {
            const next = e.state?.tab ?? tabFromPath();
            setTab(next);
        };
    ...
    return (_jsxs("div", { className: "app", children: [_jsx(ToastContainer, { toasts: toasts, onDismiss: dismissToast }), _jsxs("aside", { className: "sidebar", children: [_jsxs("div", { className: "sidebar-brand", children: [_jsx("div", { className: "sidebar-brand-icon", children: _jsx(VaultIcon, { size: 22 }) }), _jsx("span", { className: "sidebar-brand-text", children: "Magnus" })] }), _jsxs("div", { className: "sidebar-section", children: [_jsx("div", { className: "sidebar-label", children: "Main menu" }), _jsx("nav", { className: "sidebar-nav", children: visibleTabs.map((t) => {
                            ...
                            flexWrap: "wrap"
                                ...
                                }, children: [_jsx(ShieldIcon, { size: 13 }), "Admin"] }))] }), tab === "Dashboard" && (_jsx(DashboardPage, { dashboardLoading: dashboardLoading, isAdmin: isAdmin, dashboard: dashboard, userDashboard: userDashboard, dashboardError: dashboardError, CalendarIcon: CalendarIcon, dashboardRange: dashboardRange, setDashboardRange: setDashboardRange, DASHBOARD_RANGE_OPTIONS: DASHBOARD_RANGE_OPTIONS, DASHBOARD_RANGE_LABELS: DASHBOARD_RANGE_LABELS, refreshDashboard: refreshDashboard, RefreshCwIcon: RefreshCwIcon, UsersIcon: UsersIcon, LayersIcon: LayersIcon, KeyIcon: KeyIcon, ShieldAlertIcon: ShieldAlertIcon, ShieldIcon: ShieldIcon, ActivityIcon: ActivityIcon, SearchIcon: SearchIcon, ChevronIcon: ChevronIcon, auditSearch: auditSearch, setAuditSearch: setAuditSearch, auditActionFilter: auditActionFilter, setAuditActionFilter: setAuditActionFilter, formatActionLabel: formatActionLabel, formatDate: formatDate, users: users, DEFAULT_PAGE_SIZE: DEFAULT_PAGE_SIZE, dashboardAuditPage: dashboardAuditPage, setDashboardAuditPage: setDashboardAuditPage, userRecentActivityPage: userRecentActivityPage, setUserRecentActivityPage: setUserRecentActivityPage, session: session })), tab === "Users" && (_jsx(UsersPage, { setShowCreateUser: setShowCreateUser, PlusIcon: PlusIcon, users: users, DEFAULT_PAGE_SIZE: DEFAULT_PAGE_SIZE, usersPage: usersPage, setUsersPage: setUsersPage, onSelectUser: onSelectUser, selectedUser: selectedUser, getInitials: getInitials, ShieldIcon: ShieldIcon, canControlSecurity: canControlSecurity, onForceLogoutUser: onForceLogoutUser, isBusy: isBusy, session: session, formatDate: formatDate, setSelectedUser: setSelectedUser, editUserFirstName: editUserFirstName, setEditUserFirstName: setEditUserFirstName, editUserLastName: editUserLastName, setEditUserLastName: setEditUserLastName, editUserEmail: editUserEmail, setEditUserEmail: setEditUserEmail, editUserPhoneNumber: editUserPhoneNumber, setEditUserPhoneNumber: setEditUserPhoneNumber, selectedRoleIds: selectedRoleIds, onRequestUserAccessRoleChange: onRequestUserAccessRoleChange, roles: roles, ChevronIcon: ChevronIcon, onResetPassword: onResetPassword, setDeleteUserId: setDeleteUserId, onDoneUserDetails: onDoneUserDetails, isUserProfileDirty: isUserProfileDirty, resetUserId: resetUserId, setResetUserId: setResetUserId, showResetPassword: showResetPassword, setShowResetPassword: setShowResetPassword, resetNewPassword: resetNewPassword, setResetNewPassword: setResetNewPassword, LockIcon: LockIcon, EyeOffIcon: EyeOffIcon, EyeIcon: EyeIcon, onConfirmResetPassword: onConfirmResetPassword, resetPasswordError: resetPasswordError, pendingRoleChange: pendingRoleChange, setPendingRoleChange: setPendingRoleChange, onConfirmUserAccessRoleChange: onConfirmUserAccessRoleChange, deleteUserId: deleteUserId, deleteUserTarget: deleteUserTarget, onConfirmRemoveUser: onConfirmRemoveUser })), tab === "Roles" && (() => {
                        const ROLE_META = {
                            admin: {
                                label: "Admin",
