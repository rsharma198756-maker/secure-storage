...
type IpAccessDecisionReason = "disabled" | "not_allowlisted";
...
const defaultEmergencyShutdownState = (): EmergencyShutdownState => ({
  active: false,
  reason: null,
  startedAt: null,
  endedAt: null,
...
const getEmergencyShutdownState = async (): Promise<EmergencyShutdownState> => {
  const now = Date.now();
  if (
    emergencyShutdownStateCache &&
    now - emergencyShutdownStateCache.loadedAt < EMERGENCY_SHUTDOWN_CACHE_MS
  ) {
    return emergencyShutdownStateCache.value;
  }

  let value: EmergencyShutdownState;
  try {
    const res = await pool.query(
      `
      SELECT
    ...
    if (error?.code === "42P01") {
      if (!warnedMissingEmergencyShutdownTable) {
        ...
        app.log.warn(
          "emergency_shutdown_controls table is missing; emergency shutdown is disabled until migrations are applied"
        );
      }
...
const getIpAccessDecision = (state: IpAccessState, ipAddress?: string | null) => {
  if (!ipAddress) {
    return { allowed: true, reason: null as IpAccessDecisionReason | null };
  }
...
const getIpBlockedPayload = (reason: IpAccessDecisionReason) =>
  reason === "disabled" ? IP_DISABLED_PAYLOAD : IP_NOT_ALLOWLISTED_PAYLOAD;

const sendIpAccessDenied = (reply: any, reason: IpAccessDecisionReason) =>
  reply.code(403).send(getIpBlockedPayload(reason));
...
const isAdminUser = async (client: DbClient, userId: string) => {
