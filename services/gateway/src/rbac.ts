...
let cached:
  | {
      enforcer: Awaited<ReturnType<typeof newEnforcer>>;
      loadedAt: number;
    }
  | undefined;
...
const loadPolicies = async (pool: Pool) => {
  ...
  for (const row of rolePerms.rows) {
    const [obj, act] = (row.permission_key as string).split(":");
    if (!obj || !act) continue;
...
export const getEnforcer = async (pool: Pool) => {
  const now = Date.now();
