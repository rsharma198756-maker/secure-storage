...
const getSecurityControls = async () => {
      ...
      `);
        ...
        if (error?.code === "42P01") {
            if (!warnedMissingSecurityControlsTable) {
                ...
                app.log.warn("security_controls table is missing; security controls are temporarily disabled until migrations are applied");
            }
            value = {
                globalLogoutAfter: null,
                tapOffActive: false,
...
const storageFetch = (path, init, context) => fetch(`${config.storageUrl}${path}`, {
    ...init,
    headers: {
        ...(init.headers ?? {}),
...
const hardDeleteUserAccount = async (client, userId, options) => {
    ...
    await client.query("DELETE FROM item_grants WHERE subject_type = 'user' AND subject_id = $1", [userId]);
    ...
    try {
        await client.query("UPDATE refresh_tokens SET revoked_by_user_id = NULL WHERE revoked_by_user_id = $1", [userId]);
    }
    catch (error) {
    ...
    try {
        await client.query("UPDATE security_controls SET tap_off_by_user_id = NULL, updated_at = now() WHERE tap_off_by_user_id = $1", [userId]);
    }
    catch (error) {
        if (!isMissingRelationOrColumn(error))
            throw error;
    ...
    const verifyRes = await client.query("SELECT 1 FROM users WHERE email = $1 LIMIT 1", [target.email]);
...
const requirePermission = async (request, reply, permissionKey) => {
...
const requireSecurityControlAccess = async (request, reply, options) => {
    if (!ensureSecurityControlsEnabled(reply))
        return false;
    if (!(await requirePermission(request, reply, "security:control")))
        return false;
    if (options?.requireStepUp === false)
        return true;
    if (!requireSecurityActionToken(request, reply))
        return false;
    return true;
};
const ensureSecurityControlsEnabled = (reply) => {
    if (config.securityControlsEnabled)
        return true;
    reply.code(404).send({ error: "not_found" });
    return false;
};
const getUserAuthState = async (userId) => {
    const res = await pool.query("SELECT id, status, force_logout_after FROM users WHERE id = $1 LIMIT 1", [userId]);
    if (!res.rowCount)
        return null;
    return {
        id: res.rows[0].id,
...
const isAccessTokenRevokedByCutoff = (claims, userCutoff, globalCutoff) => {
    if (typeof claims.iat !== "number")
        return false;
    const tokenIat = claims.iat;
    const toEpochSeconds = (value) => value ? Math.floor(value.getTime() / 1000) : null;
    const userCutoffEpoch = toEpochSeconds(userCutoff);
    const globalCutoffEpoch = toEpochSeconds(globalCutoff);
    if (userCutoffEpoch !== null && tokenIat <= userCutoffEpoch)
        return true;
    if (globalCutoffEpoch !== null && tokenIat <= globalCutoffEpoch)
        return true;
    return false;
};
const requireSecurityActionToken = (request, reply) => {
    const userId = request.userId;
    if (!userId) {
        reply.code(401).send({ error: "unauthorized" });
        return null;
    ...
    let claims;
    try {
        claims = verifySecurityActionToken(token);
    }
    catch {
        reply.code(401).send({ error: "security_action_token_invalid" });
        return null;
    }
    if (claims.kind !== "security_action" ||
...
const sanitizeArchivePath = (relativePath) => relativePath
    .split("/")
    .filter(Boolean)
    .map((segment) => sanitizePathSegment(segment))
    .join("/");
const toContentDispositionFilename = (filename) => encodeURIComponent(filename);
const guessContentTypeFromName = (name) => {
...
const resolveContentType = (name, value) => {
...
app.get("/ready", async (_request, reply) => {
    ...
    try {
        const res = await storageFetch("/health", { method: "GET" }, { scope: "health.check" });
...
app.get("/internal/storage-health", async (_request, reply) => {
    try {
        const res = await storageFetch("/health", { method: "GET" }, { scope: "health.proxy" });
...
const shouldLimit = async (key, max, windowSeconds) => {
    const redis = await getRedis();
...
const otpRequestLimiter = async (email, ip) => {
    const limitEmail = await shouldLimit(`otp:req:email:${email}`, config.otpRequestMax, config.otpRequestWindowSeconds);
    if (limitEmail)
        return true;
    if (ip) {
        return shouldLimit(`otp:req:ip:${ip}`, config.otpRequestMax, config.otpRequestWindowSeconds);
    }
    return false;
};
const otpVerifyLimiter = async (email, ip) => {
    const limitEmail = await shouldLimit(`otp:verify:email:${email}`, config.otpVerifyMax, config.otpVerifyWindowSeconds);
    if (limitEmail)
        return true;
    if (ip) {
        return shouldLimit(`otp:verify:ip:${ip}`, config.otpVerifyMax, config.otpVerifyWindowSeconds);
    }
    return false;
};
const securityStepUpRequestLimiter = async (email, ip) => {
    const limitEmail = await shouldLimit(`otp:stepup:req:email:${email}`, config.otpRequestMax, config.otpRequestWindowSeconds);
    if (limitEmail)
        return true;
    if (ip) {
        return shouldLimit(`otp:stepup:req:ip:${ip}`, config.otpRequestMax, config.otpRequestWindowSeconds);
    }
    return false;
};
const securityStepUpVerifyLimiter = async (email, ip) => {
    const limitEmail = await shouldLimit(`otp:stepup:verify:email:${email}`, config.otpVerifyMax, config.otpVerifyWindowSeconds);
    if (limitEmail)
        return true;
    if (ip) {
        return shouldLimit(`otp:stepup:verify:ip:${ip}`, config.otpVerifyMax, config.otpVerifyWindowSeconds);
    }
    return false;
};
const ensureAuthFlowAvailable = async (reply) => {
    if (!config.securityControlsEnabled)
        return true;
...
const getUserProfile = async (userId) => {
    ...
    const rolesRes = await pool.query(`SELECT r.name FROM user_roles ur JOIN roles r ON r.id = ur.role_id WHERE ur.user_id = $1`, [userId]);
    const roles = rolesRes.rows.map((r) => r.name);
    const permsRes = await pool.query(`SELECT DISTINCT p.key FROM user_roles ur
     JOIN role_permissions rp ON rp.role_id = ur.role_id
     JOIN permissions p ON p.id = rp.permission_id
     WHERE ur.user_id = $1`, [userId]);
    const permissions = permsRes.rows.map((p) => p.key);
    return {
        roles,
        permissions,
...
app.post("/auth/refresh", async (request, reply) => {
    ...
    if (isAccessTokenRevokedByCutoff({ sub: record.user_id, email: userRes.rows[0].email, iat: issuedAtSeconds }, userRes.rows[0].force_logout_after
        ? new Date(userRes.rows[0].force_logout_after)
        : null, controls.globalLogoutAfter)) {
        reply.code(401).send({ error: "session_revoked" });
        return;
    }
...
app.get("/auth/me", async (request, reply) => {
    ...
    const userRes = await pool.query("SELECT id, email, status, force_logout_after FROM users WHERE id = $1", [userId]);
    if (userRes.rowCount === 0) {
        reply.code(401).send({ error: "invalid_user" });
        return;
    }
    ...
    if (accessClaims &&
        isAccessTokenRevokedByCutoff(accessClaims, userRes.rows[0].force_logout_after
            ? new Date(userRes.rows[0].force_logout_after)
            : null, controls.globalLogoutAfter)) {
        reply.code(401).send({ error: "session_revoked" });
        return;
...
app.post("/admin/security/step-up/verify", async (request, reply) => {
    ...
    const userId = request.userId;
    const userRes = await pool.query("SELECT id, email, status FROM users WHERE id = $1 LIMIT 1", [userId]);
    if ((userRes.rowCount ?? 0) === 0) {
        reply.code(401).send({ error: "invalid_user" });
        return;
    }
...
app.post("/admin/users", async (request, reply) => {
    ...
    try {
        await client.query("BEGIN");
        const existing = await client.query("SELECT id, status FROM users WHERE email = $1 LIMIT 1", [email]);
       ...
       RETURNING id`, [
        ...
        if (roleRes.rowCount && roleRes.rowCount > 0) {
            await client.query("INSERT INTO user_roles (user_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [newUserId, roleRes.rows[0].id]);
        }
        await client.query("COMMIT");
    }
    catch (error) {
...
app.post("/admin/users/:id/reset-password", async (request, reply) => {
    ...
    const passwordHash = await hashPassword(body.password);
    const res = await pool.query("UPDATE users SET password_hash = $1, updated_at = now() WHERE id = $2 RETURNING id", [passwordHash, id]);
    if ((res.rowCount ?? 0) === 0) {
        reply.code(404).send({ error: "user_not_found" });
        return;
    }
...
app.post("/admin/users/:id/reset-roles", async (request, reply) => {
    ...
    const roleRes = await pool.query("SELECT id FROM roles WHERE name = 'viewer' LIMIT 1");
    if (roleRes.rowCount !== null && roleRes.rowCount > 0) {
        await pool.query("INSERT INTO user_roles (user_id, role_id) VALUES ($1,$2)", [id, roleRes.rows[0].id]);
    }
    invalidateEnforcer();
    await writeAuditLog({
        actorUserId: request.userId,
...
app.get("/admin/roles", async (request, reply) => {
    if (!(await requirePermission(request, reply, "roles:manage")))
        return;
    const res = await pool.query("SELECT id, name, description FROM roles ORDER BY name ASC");
    reply.send(res.rows);
});
app.get("/admin/permissions", async (request, reply) => {
    if (!(await requirePermission(request, reply, "roles:manage")))
        return;
    const res = await pool.query("SELECT id, key, description FROM permissions ORDER BY key ASC");
    reply.send(res.rows);
});
...
const createItem = async (params) => {
    ...
    try {
        await client.query("BEGIN");
        if (params.parentId) {
            const parent = await client.query("SELECT id, type FROM items WHERE id = $1 AND status = 'active'", [params.parentId]);
      ...
      `, [
        ...
        await client.query("INSERT INTO item_closure (ancestor_id, descendant_id, depth) VALUES ($1,$1,0)", [id]);
        if (params.parentId) {
            await client.query(`
...
const getStorageUserContext = (request, reply) => {
    if (!request.userId) {
        reply.code(401).send({ error: "unauthorized" });
        return null;
    }
...
const presignUpload = async (context, itemId, contentType) => {
    const res = await storageFetch("/internal/presign/upload", {
        method: "POST",
...
app.get("/items", async (request, reply) => {
    if (!(await requirePermission(request, reply, "items:read")))
        return;
    const userId = request.userId;
...
app.get("/items/:id", async (request, reply) => {
    ...
    const res = await pool.query("SELECT * FROM items WHERE id = $1 AND status = 'active'", [id]);
    if (res.rowCount === 0) {
        reply.code(404).send({ error: "not_found" });
        return;
    }
...
app.patch("/items/:id", async (request, reply) => {
    if (!(await requirePermission(request, reply, "items:write")))
        return;
    const userId = request.userId;
    const { id } = request.params;
    const body = request.body;
...
app.post("/items/folder", async (request, reply) => {
    ...
    if (body.parentId) {
        const parentRes = await pool.query("SELECT 1 FROM items WHERE id = $1 AND type = 'folder' AND status = 'active'", [body.parentId]);
...
app.put("/items/:id/content", async (request, reply) => {
    ...
    const nextNameHeader = fileNameHeader && !Array.isArray(fileNameHeader)
        ? fileNameHeader
        : undefined;
    const contentTypeHeader = request.headers["content-type"];
    const nextContentType = contentTypeHeader && !Array.isArray(contentTypeHeader)
...
app.get("/items/:id/download", async (request, reply) => {
    ...
    if (item.type === "file") {
        ...
        reply.header("content-type", resolveContentType(item.name, item.content_type ?? upstream.headers.get("content-type")));
        reply.header("content-disposition", `${disposition}; filename*=UTF-8''${toContentDispositionFilename(sanitizePathSegment(item.name))}`);
    ...
    reply.header("content-disposition", `attachment; filename*=UTF-8''${toContentDispositionFilename(`${sanitizePathSegment(item.name)}.zip`)}`);
    void (async () => {
...
app.post("/items/:id/share", async (request, reply) => {
    if (!(await requirePermission(request, reply, "items:share")))
        return;
    const userId = request.userId;
    const { id } = request.params;
    const body = request.body;
...
app.get("/items/:id/shares", async (request, reply) => {
    ...
    const res = await pool.query("SELECT * FROM item_grants WHERE item_id = $1 ORDER BY granted_at DESC", [id]);
    reply.send(res.rows);
});
app.delete("/items/:id", async (request, reply) => {
    if (!(await requirePermission(request, reply, "items:delete")))
        return;
    const userId = request.userId;
    const { id } = request.params;
    ...
    const res = await pool.query("SELECT 1 FROM items WHERE id = $1 AND status = 'active'", [id]);
    if (res.rowCount === 0) {
        reply.code(404).send({ error: "not_found" });
        return;
    }
