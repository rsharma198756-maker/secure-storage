...
app.post("/admin/users", async (request, reply) => {
    ...
    const insertRes = await pool.query(`INSERT INTO users (email, password_hash, status, first_name, last_name, email_verified_at)
    ...
    if (roleRes.rowCount && roleRes.rowCount > 0) {
        await pool.query("INSERT INTO user_roles (user_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [newUserId, roleRes.rows[0].id]);
        invalidateEnforcer();
    }
    await writeAuditLog({
        actorUserId: request.userId,
