...
const credentials = config.s3AccessKey && config.s3SecretKey
    ? {
...
const ensureBucket = async () => {
    if (!config.s3AutoCreateBucket || bucketReady)
        return;
    try {
...
export const presignInternalDownload = async (key) => {
    await ensureBucket();
    const command = new GetObjectCommand({
        Bucket: config.s3Bucket,
        Key: key
...
export const getObject = async (key) => {
    await ensureBucket();
    const result = await s3Internal.send(new GetObjectCommand({
        Bucket: config.s3Bucket,
        Key: key
    }));
    return {
